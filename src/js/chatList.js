...
window.onload = function() {
    ...
    axios.get('http://3.35.114.140/api/auth/me', {headers}).then((response) => {
        ...
        if(response.status === 200) {
            ...
            axios.get('http://3.35.114.140/api/auth/me/getChatRooms', {headers}).then((chatRoomsResponse) => {
...
function createChatRoom(userSeq) {
    ...
    axios.post('http://3.35.114.140/api/chatRooms', data, {headers}).then((response) => {
        if(response.status === 200) {
            if(response.data) {
